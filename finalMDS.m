...
accuracy_cMDS = max(1-1/N*sum(label==condition),1/N*sum(label==condition));
wellClassifiedcMDS = subjects(condition==label)-1
misClassifiedcMDS = subjects(condition~=label)-1
...
for nb_init=1:N_init
    %s=rng(2);
    %init = rand(N,2);
    [points,stressM,disparitiesM] = mdscale(distance_matrix,2,"Criterion","metricstress","Start","cmdscale");
...
for nb_init=1:N_init
    %s=rng(2);
    %init = rand(N,2);
    [points,stressM,disparitiesN] = mdscale(distance_matrix,2,"Criterion","stress","Start","cmdscale");
